...
#error "Include metal/mutex.h instead of metal/baremetal/mutex.h"
#endif
...
#define METAL_MUTEX_INIT(m) \
  { ATOMIC_VAR_INIT(METAL_MUTEX_UNLOCKED) }
/*
...
static inline void __metal_mutex_deinit(metal_mutex_t *mutex) { (void)mutex; }
...
static inline void __metal_mutex_acquire(metal_mutex_t *mutex) {
  ...
  while (
      !atomic_compare_exchange_weak(&mutex->v, &unlocked, METAL_MUTEX_LOCKED)) {
    ;
  }
}
